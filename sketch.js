...
let paddle_x,paddle_y,paddle_width_paddle_height,paddle_dx,paddle_init_x,paddle_init_y;
...
function setup() {
  ...
  for(var i=0;i<4;i++){
    let row =[];
    for(var j=0; j<4 ; j++){
      row.push({x : (i*90) + 10, y : (j*50) + 25 ,w: 85 ,h: 30,v:true })
...
function draw () {
  ...
  //Show user has won the game
  if(scores === (grid.length*grid[0].length) ){
    ball_dx=0;
    ball_dy=0;
    text("Congrats! You've won", (width/2)-50,(height/2));
  }


  //ball touching brick logic
